...
class GeminiCollector:
    ...
    def _parse_gemini_response(self, text: str) -> Dict:
        ...
        # Clean up "unknown" values and convert confidence to percentage
        for key in result:
            if result[key] and result[key].lower() in ["unknown", "n/a", "not available", ""]:
                result[key] = None if key != "confidence" else "low"
        
        # Convert confidence to percentage
        confidence_map = {"high": 95, "medium": 75, "low": 50}
        if result.get("confidence"):
            result["confidence"] = confidence_map.get(result["confidence"].lower(), 50)
        else:
